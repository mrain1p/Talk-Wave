...
- **Live on air** — the phone-in's two doors, standing only while the *Go live on the station* caller permission is on at all; with that permission off the whole cluster leaves the page rather than greying. Two quick kills in the same grammar as the Lines above: **Live Call** and **Aired Voicemails**, each closing its own door without touching the other. A door switched off here simply stays private — callers still talk to the DJ, they just don't reach the broadcast. The feature itself — the relay, the caller's switch, the wiring — has [a page of its own](on-air.md).
