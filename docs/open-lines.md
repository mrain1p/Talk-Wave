...
**A quiz is its own kind of subject.** Free text cannot carry one: given "Quiz question" as a premise the DJ announced that it had been handed a label, then invented a question with no answer in mind and told a caller who answered it that they had whiffed. So with *A quiz* selected, the DJ sets a question **and its answer** before anything airs, and both are kept. The block hands it the answer, which is what makes "right or wrong" a judgement rather than an improvisation.

The question is always about the DJ's own world — the show, itself, the records it has played tonight, something it said on air earlier — never chart positions or release years. Two reasons: the DJ genuinely knows those answers, and a listener who has had the show on all evening can actually win. It is also told that **an answer is not a request**, because a caller saying a song title was read as one and got a track queued at them.

**It never outlasts the programme.** The station publishes its week as an hour grid, so Talk Wave knows when the DJ currently on air gives way to the next one, and a line is cut to that boundary. A premise opened by one DJ never survives into another's show. In the last few minutes of a show nothing is shortened at all, since cutting a line that fine would air its invitation and its sign-off back to back.
