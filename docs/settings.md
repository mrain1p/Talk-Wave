...
| **Let signed-in listeners start one** | Puts the segment button in the player's own ribbon for anyone holding a guest code, not just you. **Off by default**, and worth thinking about: it is the only control on that page that reaches the broadcast, and a guest code is shared more freely than an admin password. Your own dashboard button is unaffected |
...
| **Only these DJs** | A boxed panel of chips — one per DJ, tap to tick, with **All** / **None** and a live count. None ticked means whoever is on air. This paces the AUTOMATIC cadence only — **a press always runs**, because an operator holding the button has already made that decision. Separate from the per-subject aim above: this says who may open a line at all, that says which subjects suit whom |
...
- the **programme guide card** — the station's week, read from its public schedule through this server and cached five minutes: today's shows hour by hour, then every show with its description, its DJs and their published souls, and its times. Off by default
...
**Station tools** is the station's whole surface — 38 tools in all: 7 handed straight through from the station's MCP server, 28 served by wrappers of ours (retries, guards, the action ledger), and 3 that are never on a call line at any setting — each row saying whether a caller can reach it.
