...
**Station tools** is the station's whole surface — 41 tools in all: 7 handed straight through from the station's MCP server, 29 served by wrappers of ours (retries, guards, the action ledger), and 5 that are never on a call line at any setting — each row saying whether a caller can reach it.
