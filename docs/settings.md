...
|---|---|---|
| Configuration | **Access** | Call-in access (automatic, open, guest code, admin only), the admin password for this panel, and the optional guest code for the phone. `CALLIN_ADMIN_KEY` is the recovery override. A guest-code expiry (minutes) for shared machines — the card also offers a lock button to forget the code immediately |
