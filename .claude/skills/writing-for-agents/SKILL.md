...
whose README invites exactly this. Adapted only at the end, where Talk Wave's own
conventions are named; everything above that is his.
...
The material this reference is about is already load-bearing here, so the levers land on
things that exist:

- **The always-loaded pointers** are the fourteen-odd skill descriptions in
  `.claude/skills/*/SKILL.md` and the two tracked `CLAUDE.md` files
  (`agent-worker/`, `web-widget/`). Every word of those is context load on every turn.
  The root `CLAUDE.md` is deliberately untracked — it is the operator's working brief, not
  documentation of the code.
- **The environment is the source of truth** for anything a lookup can answer:
  `run_tests.py --help`, `.claude/launch.json`, the settings schema. What the docs here
  cache is what no config confesses — the reason behind a choice, and the gotcha that has
  already cost a session.
- **The tests are documents too.** This suite pins decisions with their reasons in the
  assertion message (the size and complexity ledgers, the type scale, the widget
  contract). A test whose reason has gone stale is sediment in exactly the sense above:
  prune it in the same commit that changes the behaviour, and say why in the number.
- **Commit messages are the long form.** This repo writes the reasoning into the commit
  rather than into the code, so the same pruning and leading-word discipline applies to
  them. `git log` is where a future agent finds out why.

The **single source of truth** rule is the one that bites hardest here: a fact stated in a
skill, a `CLAUDE.md` and a test comment is a fact that will disagree with itself within a
month. Put it where it is enforced, and point at it from the other two.
