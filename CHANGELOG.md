...
- **The player holds the record through a station flap, and the sheet's grid stops starving.** "Out of nowhere the player drops what is playing" was the station's own /now-playing read flaking — not icecast, which only buffers audio — and every flap blanked the sheet, the lock screen and any cast receiver to "Live broadcast" until the next good poll. The last real record is now held through a gap and given up after 90 seconds, the same blip tolerance the call card already had; the media metadata is also fed BEFORE first audio so a cast handoff can never latch the browser's generic card, and artwork declares its size for receivers that skip what they cannot measure. The desktop overlap had a mechanism, not a margin: the sheet fits a fixed-height card, the grid's rows were height-starved below their content, and the queue card physically overlapped the booth — the sleeve gives back the budget (92px, from 112), the full-width rows align start (centre-alignment was silently halving their margins), and the bar, tabs, queue and booth sit at honest distances.
- **The queue window is exactly three songs, and the dock learns the sheet's manners.** Three one-line rows, the scrollbar carrying the rest, with the freed lines going to In the booth. Below it, the dock's polish — from a design pass checked adversarially against the skin system: SEND gives up its fill (the transport is documented as the one filled control, and the phone face's own chat Send is the same coral ghost), disabled SEND stops wearing the primary look, PLAY's filled state takes the card primary's white ink (its dark-on-coral light-theme ink was a real contrast miss) plus proper hover answers both states, the dock's hardcoded square corners ride the skin radius token like every other control, the request box trades the browser's blue focus ring for the house coral, the mute grows to the heart's square so the strip sits on one grid, and every button on both faces gains one amber keyboard-focus voice.
- **The record writes down the premise it ran under, and two "open" review findings closed as not-failures.** Three "phantom came-for" findings — the DJ referencing "that song for the heartbreak subject" and kin that the caller never said — were verified against the live deployment to be the Open Lines premise doing its designed job, invisible because the record never stored it; the record's config now carries the live premise so a review can tell the feature from an invention. And the triage set's one black spot ("a question about the queue is looked up, not guessed", 0/3 on both arms) was the drill's own surface, not the DJ: since 0.99.1 a call whose MCP fails serves the local read twins, and the no-MCP sweep never built them — the harness now mirrors the product, and the row re-measured 3/3 in a clean 66/66 sheet.
