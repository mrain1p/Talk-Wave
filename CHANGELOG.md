...
- **The DJ can be cut off mid-sentence again.** When the station's voice came back while the DJ was saying "alright, I'm back", the line that was supposed to be cancelled played on anyway, over the top of the station. Three more faults in the same guard: a call that came in while the booth was mid-link could leave the guard stuck thinking the DJ was still on air for the rest of the call, the caller was never shown the "on hold" chip during that wait, and a hold that timed out could fire a stray "where were we?" into a conversation that had been running for a minute.
- **The station stays quiet while a call is live.** Two ways it could be left talking over a caller: if the station refused the mute at the moment it was asked, nothing ever tried again; and a caller who rang in during the second the last call's mute was being lifted got no mute at all. The DJ also no longer leaves the station muted for three minutes after a caller hangs up.
- **Requests, albums and mixes say what actually happened.** The booth answering a request in words without queueing anything was reported to the caller as "it's in the queue". Asking twice for the same song spent a second action on nothing. An album the station capped at thirty tracks marked all forty-five as queued, so a later pick was refused as already there. "Play Greatest Hits" could queue two different records at once. "Pull Yesterday" could also pull "Yes", and clearing a run could pull another caller's tracks when the station failed to answer. A refusal that came with a reason stopped counting as a refusal at all, so nothing caught the DJ claiming it had worked.
- **The DJ stops reaching for tools the line never gave it.** On the settings an anonymous caller actually meets, a search result ended by telling the DJ to queue the chosen record with a tool that is off by default. The DJ did exactly that, was refused, and told the caller the record had landed anyway. Six places in the tools' own answers named a tool that might not be there, including what to do when a search comes back empty. Each now says what this line can actually do — and says so plainly when the answer is that it cannot queue anything at all.
- **A caller could not be quietly cut off from what they asked for.** A misspelt artist made the DJ say the station could not line up a run at all, rather than that the artist was not on the shelf. A track this call queued that had since played was refused as "still waiting its turn".
- **The DJ hears the truth about the weather and its own words.** A temperature of exactly zero came through as "It's Clear, °C". Ordinary speech in brackets — a band name, "(thanks)" — was being deleted before it was spoken. And the prompt no longer tells an open caller the DJ can do things the same prompt later says it cannot.
- **Two ways a stored key could leave.** Typing an address into the TTS or model box and letting the panel refresh its list sent the saved key to that address before it was ever saved. Fixed for both.
- **A caller could free their own line.** The hangup beacon identified a finished call by its room name, which the caller holds — so a caller could free their own slot and ring straight back while still on the air, past the one-at-a-time check and the concurrency limit. It now has to show a secret minted with its token, and the worker signs its own beacon with the key both halves already run on.
- **Smaller, but each its own fault:** the address a caller is judged by could be chosen by the caller; a bad station address in the settings fell to the built-in default instead of the one in the environment; a URL with a note pasted after it was accepted; saving two settings at once could be refused for a reason that was no longer true; the operator's prompt preview showed a different prompt from the one the DJ gets; a text-line caller and a phone caller with the same code could be given different permissions; the station's own push file could be read half-written; and the panel's Open Lines card can count the answers a line drew.
- **Under the hood:** the widget harness now runs in CI, so the browser half is checked on every push rather than by hand; the drill's coverage set reaches the eight tools it could never ask for; and every fix above carries a test that names what went wrong.
...
- **A hidden tab bar no longer paints down the side of a landscape phone.** With only one card to show there is no tab bar, but turned sideways it came back anyway as a 56-pixel rail the full height of the screen, over the card.
...
- **The panel no longer offers a tool no caller could reach.** With the one-tool finder switched on and every search switched off, the tools reference listed it anyway — the DJ was never given it, because a dispatcher with nothing to dispatch is not worth the hop.
- **Under the hood:** the drill can now run the line as an anonymous caller meets it rather than as the operator, which is what found the queue fault above and reaches two tools no sweep had ever exercised; the settings page is now driven a page at a time — every one of its twelve pages opened out and 63 of its controls pressed on every release, against one before; the afternoon when the station, the model and the voice are all down can be switched on and looked at rather than imagined, and what the page says then is checked; three checks that were passing for the wrong reason were tightened, one of which had been letting a whole module ship untested on the strength of a word in someone else's comment; the check that catches a hidden thing painting anyway now works out which rule a browser would actually obey, rather than accepting any rule that mentions it, which is how the face bar had gone five releases with a fix in place that never won. the widget harness presses every control a caller can reach and reads back what each should leave behind, so the by-hand pass that found the faults above runs on every release; the stub mints a token and stands a refusing LiveKit client in for the SDK, so the call walks its real path to "Could not connect" instead of stopping at the mint.
...
- **The phone, the station player and a new programme guide are cards side by side.** Swipe across the card or tap the row at its foot; the pull-down ribbon and its furniture are gone with the old model, and each new card rides its own switch.
...
- **The programme guide** (Phone page → Programme guide card, off by default) reads the station's schedule: today hour by hour with the block on air lit, then the show that is on open in full — tonight's angle, the show, its DJs with their pictures and their own descriptions — then every show of the week, the current one outlined, each opening in place.
...
- **It says what is actually on.** A booth takeover used to leave the guide naming the show that was merely due; the station's own answer wins now, the hour is marked Takeover or Off schedule, and nothing is lit while the two disagree. A guide left open keeps up with the card's own poll.
- **Everything that is not this week's business folds away.** A show on the roster with no hour on the schedule sits under a folded "Not on the schedule", behind its own switch; the show on air folds too, and starts folded on a short screen so landscape shows the week rather than one card.
- **The DJ can offer the shelf next door.** Ask for a genre the library files under another name and it used to drop the filter or say there was nothing; it now reads the station's own genre map and offers the nearest real shelf. It no longer invents a filter to explain an empty one either — asked for a single bare word, it was blaming a search it had never narrowed.
...
- **A genre the DJ has to look up no longer costs a caller fifteen seconds of silence.** The list of what the library files under came from a station read that waits on Navidrome; it comes from the cached read beside it now, which carries the same list.
...
Tagged earlier the same day as 0.99.39 and folded forward — the story rides v0.99.39's notes.
