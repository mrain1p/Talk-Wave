...
- **Under the hood:** every fix above carries a test that names what went wrong, and the checks that find them got a great deal sharper. The browser harness runs in CI, presses every control a caller can reach, drives the settings page a page at a time — twelve pages opened out and 63 controls pressed on every release, against one before — and can now be shown the afternoon when the station, the model and the voice are all down, to check what the page says then. The drill runs the line as an anonymous caller meets it rather than as the operator, which is what found the queue fault above, and its coverage set reaches ten tools it could never ask for. And four checks that had been passing for the wrong reason were tightened: one was letting a whole module ship untested on a word in someone else's comment, and one is why a hidden face bar went five releases with a fix in place that never won.
